...
## Update, 24 September 2026: family consents

One new table: `family_consents`. One row is one patient's permission for one family account: the relationship (a label), the shared parts (`visits`, `care_team`, `care_plan`), who recorded it, when it ends (empty means until withdrawn), and who withdrew it and when. A row is written once and never edited; to change what is shared, the row is withdrawn and a new one is written. Foreign keys are `Restrict`. Run `npx prisma migrate dev --name add_family_consents`, then `npx prisma db seed` (adds the two permissions, the family role set, `demo.family@cheliv.test` and Eleanor's consent). See `docs/FAMILY_PORTAL.md`. (Earlier this same day `patients.user_id` was added for the patient portal, migration `add_patient_account_link`.)
