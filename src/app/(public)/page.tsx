...
/*
  Homepage
  ========
  Every specific claim on this page is either generic to the home-health
  industry (illustrative, and labelled as such) or written to need no
  organization-specific fact at all. No staff, no certifications, no
  statistics, no testimonials - see docs/PHASE_0_ARCHITECTURE.md section 2.

  Layout is deliberately varied rather than a repeated "three cards" - an
  asymmetric hero, a numbered process (a genuine sequence), an editorial
  two-column section, a plain list for services, and an accordion FAQ.
*/
...
const serviceCategories = [
  {
    ...
    body: "Clinical care delivered at home by a licensed nurse.",
  },
  {
    title: "Physical therapy",
    body: "Regaining strength, balance and mobility after an illness, injury or surgery.",
  },
  {
    title: "Occupational therapy",
    body: "Relearning the everyday tasks that make independent living possible.",
  },
  {
    title: "Speech therapy",
    body: "Support for communication, swallowing and related recovery.",
  },
  {
    title: "Medical social services",
    body: "Help navigating the practical and emotional side of a health change.",
  },
  {
    title: "Home health aide",
    body: "Help with the daily tasks of living, at the pace the patient needs.",
  },
] as const;
...
export default function HomePage() {
  return (
    <>
      {/* HERO — asymmetric two column, not the generic centered-headline default */}
      <section className="mx-auto grid max-w-6xl items-center gap-12 px-6 py-16 lg:grid-cols-[1.1fr_0.9fr] lg:py-24">
        <div>
          ...
          <p className="mt-6 max-w-lg text-body-lg text-slate">
      ...
      {/* HOW WE CARE — a genuine sequence, so numbering earns its place here */}
      <section className="border-t border-border bg-white px-6 py-16 lg:py-24">
        <div className="mx-auto max-w-6xl">
          ...
          <ol className="mt-12 grid gap-10 sm:grid-cols-2 lg:grid-cols-4">
            {careSteps.map((step) => (
              <li key={step.number}>
                <p className="font-display text-h3 text-marigold">
                  {step.number}
                </p>
                <p className="mt-2 text-h4 font-semibold text-ink">
                  {step.title}
                </p>
      ...
      {/* SERVICES — a plain descriptive list, not a repeated card grid;
          clearly marked as illustrative until the organization confirms
          its actual service lines */}
      <section className="border-t border-border bg-sage px-6 py-16 lg:py-24">
        <div className="mx-auto max-w-6xl">
          ...
          <ul className="mt-10 divide-y divide-border-strong border-y border-border-strong">
            {serviceCategories.map((service) => (
              ...
              >
                <p className="text-h4 font-semibold text-ink sm:w-64 sm:flex-none">
                  {service.title}
                </p>
      ...
      {/* WHO WE SERVE — editorial two column, not another card grid */}
      <section className="border-t border-border bg-white px-6 py-16 lg:py-24">
      ...
      {/* REQUEST CARE CTA */}
      <section className="border-t border-border bg-pine px-6 py-16 text-white lg:py-20">
        <div className="mx-auto flex max-w-6xl flex-col items-start justify-between gap-6 lg:flex-row lg:items-center">
          ...
          <Link
            ...
            className={buttonVariants({
              variant: "secondary",
              size: "lg",
              className: "bg-white text-pine hover:bg-sage",
            })}
          >
            Request care
          </Link>
      ...
      {/* FAQ — accordion, native <details>, no extra JS needed */}
      <section className="border-t border-border bg-white px-6 py-16 lg:py-24">
            ...
            {faqs.map((item) => (
              <details key={item.q} className="group py-5">
                <summary className="flex cursor-pointer list-none items-center justify-between text-h4 font-medium text-ink [&::-webkit-details-marker]:hidden">
                  ...
                  <span
                    aria-hidden="true"
                    className="ml-4 text-h4 font-normal text-slate group-open:hidden"
                  >
                    +
                  </span>
                  <span
                    aria-hidden="true"
                    className="ml-4 hidden text-h4 font-normal text-slate group-open:inline"
                  >
                    −
                  </span>
                </summary>
