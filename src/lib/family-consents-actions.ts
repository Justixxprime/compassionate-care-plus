// src/lib/family-consents-actions.ts
"use server";

// Thin wrappers between the browser and src/lib/family-consents.ts, exactly
// like document-grants-actions.ts: work out WHO is asking (from the session
// cookie, on the server), hand the request to the service layer where every
// real rule lives, and turn the answer into something a form can show.
//
// Nothing here decides access. A Server Action is a public HTTP endpoint:
// anyone signed in can call it directly with any values, skipping the form,
// which is why createConsent and revokeConsent check the permission, the
// patient and the person on every call.

import { revalidatePath } from "next/cache";
import { getCurrentUser } from "@/lib/auth/session";
import { AuthorizationError } from "@/lib/auth/authorize";
import { createConsent, revokeConsent } from "@/lib/family-consents";

export interface ConsentActionResult {
  ok: boolean;
  error?: string;
}

const SIGNED_OUT = "Your session has expired. Sign in again.";
const NO_PERMISSION = "You do not have permission to do that.";

async function run(
  work: (userId: string) => Promise<{ ok: boolean; error?: string }>,
): Promise<ConsentActionResult> {
  const user = await getCurrentUser();
  if (!user) return { ok: false, error: SIGNED_OUT };

  try {
    const result = await work(user.id);
    if (!result.ok) return { ok: false, error: result.error };
    revalidatePath("/consents");
    return { ok: true };
  } catch (err) {
    if (err instanceof AuthorizationError) {
      return { ok: false, error: NO_PERMISSION };
    }
    throw err;
  }
}

const text = (data: FormData, key: string) => String(data.get(key) ?? "");

export async function createConsentAction(formData: FormData): Promise<ConsentActionResult> {
  return run((userId) =>
    createConsent(userId, {
      patientId: text(formData, "patientId"),
      familyUserId: text(formData, "familyUserId"),
      relationship: text(formData, "relationship"),
      scopes: formData.getAll("scopes").map((v) => String(v)),
      duration: text(formData, "duration"),
      confirmed: text(formData, "confirmed"),
    }),
  );
}

export async function revokeConsentAction(consentId: string): Promise<ConsentActionResult> {
  return run((userId) => revokeConsent(userId, consentId));
}
